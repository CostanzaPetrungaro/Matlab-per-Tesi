...
%We can see the dependence of the response on n in the figure 1

figure (1)
plot(-EpsilonZ1, -SigmaZ1, 'r', 'LineWidth', 2)
hold on
...
title('Axial stress \sigma_z VS the axial strain \epsilon_z', 'FontSize', 14)
hold on
% Black line x = 0
...
hold on
legend({'n = 0.1', 'n = 0.3', 'n = 0.5', 'n = 0.7', 'n = 0.9'});
...
for i = 1:length(n)
    alpha0(i) = 1/(m - n(i))*(sqrt((m * n(i))/(m - n(i)+1))-n(i));
    SFirstDerivativeAlpha0(i) = 1/R1 * ((m - n(i))* alpha0(i) + n(i))/(alpha0(i)^(1-n(i))*(1 - alpha0(i))^(m + 1));
    DeltaSigmaZ(i) = sqrt((6*D1)/((1-k)^2*SFirstDerivativeAlpha0(i)));
end 

figure(3)
plot(n, DeltaSigmaZ, 'b', 'LineWidth', 2)
hold on
grid on
grid minor
xlim([0  1])
...
title('Overstress \Delta\sigma_z - n', 'FontSize', 14)
hold on
% Black line x = 0
plot([0 0], ylim, 'k', 'LineWidth', 1)
% Black line y = 0
